...
def main() -> None:
    ...
    constraint_embedder = ConstraintEmbedder(
    ...
    pairformer_stack = PairformerStack(c_s=384, c_z=128, n_blocks=48, n_heads=16, dropout=0.25)
    ...
    constraint_embedder.load_state_dict(_sub_state(state, "constraint_embedder"), strict=True)
    ...
    sub_idx = torch.randint(0, 4, size=(n_tok, n_tok), dtype=torch.long)
    substructure = F.one_hot(sub_idx, num_classes=4).float()
    constraint_feature = {
        "contact": torch.randn(n_tok, n_tok, 2),
        "pocket": torch.randn(n_tok, n_tok, 1),
        "contact_atom": torch.randn(n_tok, n_tok, 2),
    ...
    with torch.no_grad():
        ...
        z_pocket = constraint_embedder.pocket_z_embedder(feat["constraint_feature"]["pocket"])
        z_contact = constraint_embedder.contact_z_embedder(feat["constraint_feature"]["contact"])
        z_contact_atom = constraint_embedder.contact_atom_z_embedder(feat["constraint_feature"]["contact_atom"])
        z_substructure = constraint_embedder.substructure_z_embedder(feat["constraint_feature"]["substructure"])
        z_constraint = z_pocket + z_contact + z_contact_atom + z_substructure
        ...
        z_init = z_init_lin + z_relpos + z_token_bond + z_constraint
    ...
    payload = {
        ...
        "z_pocket": z_pocket.tolist(),
        "z_contact": z_contact.tolist(),
        "z_contact_atom": z_contact_atom.tolist(),
        "z_substructure": z_substructure.tolist(),
        "z_constraint": z_constraint.tolist(),
